...
import { stocks, scrapeLogs, priceHistory, incomeStatements, balanceSheets, cashFlows, stockNews } from '../db/schema.js';
...
router.get('/stocks', async (req, res) => {
    try {
        ...
        const [stockList, countResult] = await Promise.all([
            ...
            db.select({ count: sql `count(*)` }).from(stocks),
        ]);
...
router.get('/stocks/search', async (req, res) => {
    try {
        ...
        const results = await db.select()
            .from(stocks)
            .where(sql `upper(${stocks.symbol}) LIKE ${`%${query}%`} OR upper(${stocks.name}) LIKE ${`%${query}%`}`)
            .limit(20);
 ...
 */
router.get('/stocks/:symbol/financials', async (req, res) => {
    try {
        const symbol = req.params.symbol.toUpperCase();
        const periodType = req.query.period?.toLowerCase() || 'all'; // 'annual', 'quarterly', or 'all'
 ...
 */
router.get('/stocks/:symbol/income-statements', async (req, res) => {
    try {
        const symbol = req.params.symbol.toUpperCase();
        const periodType = req.query.period; // 'annual' or 'quarterly'
        let query = db.select()
            .from(incomeStatements)
 ...
 */
router.get('/stocks/:symbol/balance-sheets', async (req, res) => {
    try {
        const symbol = req.params.symbol.toUpperCase();
 ...
 */
router.get('/stocks/:symbol/cash-flows', async (req, res) => {
    try {
        const symbol = req.params.symbol.toUpperCase();
        const periodType = req.query.period;
        const data = await db.select()
 ...
 */
router.get('/stocks/:symbol/complete', async (req, res) => {
    try {
        const symbol = req.params.symbol.toUpperCase();
        const [stockData, historyData, incomeData, balanceData, cashFlowData, newsData,] = await Promise.all([
...
router.get('/stats', async (req, res) => {
    try {
        const [totalResult, successResult, errorResult, lastLogResult, historyCountResult, newsCountResult,] = await Promise.all([
            db.select({ count: sql `count(*)` }).from(stocks),
            db.select({ count: sql `count(*)` }).from(stocks).where(eq(stocks.scrapeStatus, 'success')),
            db.select({ count: sql `count(*)` }).from(stocks).where(eq(stocks.scrapeStatus, 'error')),
            db.select().from(scrapeLogs).orderBy(desc(scrapeLogs.startTime)).limit(1),
            db.select({ count: sql `count(*)` }).from(priceHistory),
            db.select({ count: sql `count(*)` }).from(stockNews),
        ]);
